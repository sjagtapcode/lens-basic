...
export default function MediaDetails({ data }: { data: MediaType }) {
  ...
  switch(true) {
    case data?.mimeType?.startsWith('image'):
      return (
        <img width="auto" height={480} src={url} alt={data?.altTag || ''} className="max-h-[480px]" />
      )
    ...
    case data?.mimeType?.startsWith('audio'):
      return (
        <div className="flex flex-col">
          {coverUrl ?  <img width="auto" height={480} src={coverUrl} alt={data?.altTag || ''} className="max-h-[480px]" /> : ''}
